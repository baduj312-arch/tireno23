...
import {
  LayoutDashboard, ClipboardList, DollarSign, User, Settings,
  ToggleLeft, ToggleRight, Star, TrendingUp, Clock, CheckCircle,
  X, Wrench, Car, MapPin, Shield, ArrowUpRight, ArrowDownLeft,
  ChevronRight
} from 'lucide-react';
...
export default function ProviderDashboard() {
  ...
  const sosRequests = [
    ...
    },
    {
      ...
      urgency: 'urgent',
      estimated: 'GH₵120-180',
    },
  ];

  const jobs = [
    { id: 'TRN-3847', service: 'Mechanic', status: 'en_route', customer: 'Kwabena O.', price: 85, time: '10:30 AM' },
    { id: 'TRN-3842', service: 'Tow', status: 'completed', customer: 'Abena D.', price: 150, time: 'Jun 24' },
    { id: 'TRN-3839', service: 'Fuel', status: 'completed', customer: 'Yaw K.', price: 45, time: 'Jun 20' },
  ];
      ...
      <header className="sticky top-0 z-40 bg-tireno-dark/95 backdrop-blur-md border-b border-tireno-surfaceLight">
        <div className="max-w-lg mx-auto flex items-center h-14 px-4">
          ...
          <div className="ml-auto flex items-center gap-2">
            <span className={`text-xs font-medium ${isOpen ? 'text-tireno-green' : 'text-tireno-gray'}`}>
              {isOpen ? 'Open' : 'Closed'}
            </span>
            <button onClick={() => setIsOpen(!isOpen)}>
              {isOpen ? (
                <ToggleRight size={28} className="text-tireno-green" />
              ) : (
                <ToggleLeft size={28} className="text-tireno-gray" />
              )}
            </button>
          </div>
        </div>
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'dashboard' && (
            ...
            >
                ...
                <div className="relative">
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      ...
                      <p className="text-white font-bold text-3xl">GH₵765</p>
                    </div>
                    <div className="text-right">
                      <p className="text-white/80 text-sm">Jobs</p>
                ...
                {stats.map((s, i) => (
                  ...
                  >
                    <div className="flex items-center gap-2 mb-2">
                      <s.icon size={16} className="text-tireno-orange" />
                    </div>
                    <p className="text-white font-bold text-xl">{s.value}</p>
                    <p className="text-tireno-gray text-xs">{s.label}</p>
                  </motion.div>
                ))}
              </div>
                ...
                <div className="space-y-3">
                  {sosRequests.map((sos, i) => (
                    ...
                    >
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2">
                          <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                            sos.urgency === 'urgent' ? 'bg-tireno-red/20' : 'bg-tireno-orange/20'
                          }`}>
                          ...
                          <div>
                            ...
                            <p className="text-tireno-gray text-xs">{sos.id}</p>
                          </div>
                        </div>
                        {sos.urgency === 'urgent' && (
                      ...
                      <div className="flex items-center gap-2 text-tireno-gray text-xs mb-3">
                        <MapPin size={12} />
                        <span>{sos.address}</span>
                        <span className="text-tireno-orange">{sos.distance}</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-tireno-gray text-xs">Est: {sos.estimated}</span>
                        <div className="flex gap-2">
                          <button
                            onClick={() => setShowBidModal(true)}
                            className="bg-tireno-orange text-white text-xs font-bold px-4 py-2 rounded-lg"
                          >
                            Place Bid
                          </button>
                          <button className="bg-tireno-surfaceLight text-tireno-gray text-xs font-medium px-4 py-2 rounded-lg">
                            Skip
                          </button>
                        </div>
                      </div>
              ...
              <div className="card-dark p-4">
                ...
                <p className="text-tireno-gray text-[10px] mt-2">
                  Formula: Rating×0.5 + Success×0.3 + Response×0.2
                </p>
              </div>
            </motion.div>
          )}
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`text-[10px] px-2 py-0.5 rounded-full ${
                      job.status === 'en_route' ? 'bg-tireno-orange/20 text-tireno-orange' :
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-tireno-gray text-xs">{job.customer}</p>
                    </div>
                    <span className="text-white font-bold text-sm">GH₵{job.price}</span>
                  </div>
                  <p className="text-tireno-gray text-xs mt-2">{job.time}</p>
                </motion.div>
              ))}
            </motion.div>
          )}
          ...
          {activeTab === 'earnings' && (
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                <div className="card-dark p-4">
                  <p className="text-tireno-gray text-xs mb-1">This Week</p>
                  <p className="text-white font-bold text-xl">GH₵890</p>
                </div>
                <div className="card-dark p-4">
                  <p className="text-tireno-gray text-xs mb-1">This Month</p>
                  ...
                  <div key={tx.id} className="card-dark p-4 flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                      ...
                      {tx.type === 'credit' ? (
                        ...
                        <ArrowUpRight size={18} className="text-tireno-red" />
                      )}
                    </div>
          ...
          {activeTab === 'profile' && (
            ...
            >
              <div className="w-20 h-20 rounded-full gradient-orange mx-auto mb-4 flex items-center justify-center text-white font-bold text-xl">
                KO
              </div>
              ...
              <div className="grid grid-cols-3 gap-3 mb-6">
                <div className="card-dark p-3">
                  <p className="text-white font-bold text-lg">203</p>
                  <p className="text-tireno-gray text-xs">Total Jobs</p>
                </div>
                <div className="card-dark p-3">
                  <p className="text-white font-bold text-lg">4.8</p>
                  <p className="text-tireno-gray text-xs">Rating</p>
                </div>
                <div className="card-dark p-3">
                  <p className="text-white font-bold text-lg">2yr</p>
                  <p className="text-tireno-gray text-xs">Member</p>
                </div>
              </div>

              <div className="space-y-2 text-left">
                <button className="w-full card-dark p-4 flex items-center gap-3 text-left">
                  <Settings size={18} className="text-tireno-orange" />
                  <span className="text-white text-sm">Settings</span>
                  <ChevronRight size={18} className="text-tireno-gray ml-auto" />
                </button>
                <button
                  onClick={() => navigate('/login')}
                  className="w-full card-dark p-4 flex items-center gap-3 text-tireno-red text-left"
                >
                  <X size={18} />
        ...
        {showBidModal && (
          ...
          >
            <motion.div
              ...
              animate={{ y: 0 }}
              exit={{ y: 100 }}
              className="bg-tireno-surface rounded-t-2xl p-6 w-full max-w-lg"
            >
              <h3 className="text-white font-bold text-lg mb-4">Place Your Bid</h3>
              <div className="space-y-4 mb-6">
                <div>
                  <label className="text-tireno-grayLight text-sm font-medium mb-2 block">Your Price (GH₵)</label>
                  <div className="relative">
                ...
                <div>
                  <label className="text-tireno-grayLight text-sm font-medium mb-2 block">ETA (minutes)</label>
                  <div className="relative">
                    <Clock size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-tireno-gray" />
                    <input
                      type="number"
                      value={bidEta}
                    ...
                    />
                    <span className="absolute right-4 top-1/2 -translate-y-1/2 text-tireno-gray text-sm">min</span>
                  </div>
                </div>
              </div>
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 btn-ghost py-4 text-base font-medium"
                >
                  Cancel
                </button>
                <button
          ...
          {[
            ...
            return (
              ...
              >
                <item.icon
                  size={22}
                  className={active ? 'text-tireno-orange' : 'text-tireno-gray'}
                />
                <span className={`text-[10px] font-medium ${active ? 'text-tireno-orange' : 'text-tireno-gray'}`}>
                  {item.label}
                </span>
              </button>
            );
